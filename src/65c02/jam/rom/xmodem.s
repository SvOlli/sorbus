; taken from:
; https://github.com/mist64/ccgmsterm/blob/main/src/xmodem.s
;
; CCGMS Terminal
;
...
; protocol constants
XMSOH   = $01   ; Start of Heading
XMSTX   = $02   ; Start of Heading (1K blocks)
XMEOT   = $04   ; End of Transmission
XMACK   = $06   ; Acknowledge
XMNAK   = $15   ; Negative Acknowledge
XMCAN   = $18   ; Cancel
XMCRC   = 'C'   ; sent by receiver as first char instead of NAK to indicate CRC instead of checksum
...
STAT_NO_EOT_AC    = 2 ; during send, receiver did not ack the EOT signal
...
xmneot:
   ...
   .byte $2c
xmtrys:
   lda   #STAT_MAX_RETRIES
   .byte $2c
xmsync:
   lda   #STAT_SYNC_LOST
   .byte $2c
xmcmab:
   lda   #STAT_USER_ABORTED
   sta   xmstat
...
xmodem_receive:
   ...
   lda   #XMNAK   ; XMODEM: send NAK before first block
   ldx   protoc
   cpx   #PROTOCOL_XMODEM
   beq   :+
   lda   #XMCRC   ; XMODEM-CRC and -1K: send 'C' before first block
:
...
@error:
   inc   xmoend   ; count protocol errors
   lda   xmoend
   cmp   #MAX_RETRIES
   bcc   @retry2   ; loop
@abort:
   jmp   xmabrt   ; cancelled

@3:
   cmp   #XMCAN   ; cancel?
   beq   @abort
   cmp   #XMEOT   ; end of transmission
   bne   @neot
