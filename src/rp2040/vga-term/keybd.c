...
static void process_generic_report(uint8_t dev_addr, uint8_t instance, uint8_t const* report, uint16_t len) {
     ...
     if (!rpt_info) {
         uart_tx_printf( "Couldn't find the report info for this report !\r\n" );
         return;
     }
...
void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t instance, uint8_t const* desc_report, uint16_t desc_len) {
    uart_tx_printf( "HID device address = %d, instance = %d is mounted\r\n", dev_addr, instance );

    // Interface protocol (hid_interface_protocol_enum_t)
    const char* protocol_str[] = { "None", "Keyboard", "Mouse" };
    uint8_t const itf_protocol = tuh_hid_interface_protocol(dev_addr, instance);
    uart_tx_printf( "HID Interface Protocol = %s\r\n", protocol_str[itf_protocol] );
...
void tuh_hid_umount_cb(uint8_t dev_addr, uint8_t instance) {
    uart_tx_printf( "HID device address = %d, instance = %d is unmounted\r\n", dev_addr, instance);
...
void tuh_hid_report_received_cb(uint8_t dev_addr, uint8_t instance, uint8_t const* report, uint16_t len)
{
    ...
    if ( !tuh_hid_receive_report(dev_addr, instance) )
    {
        uart_tx_printf("Error: cannot request to receive report\r\n");
    }
}
