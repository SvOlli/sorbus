...
extern void writeReSID1( uint8_t A, uint8_t D );
...
volatile int32_t newSample = 0xffff;
   ...
   while ( 1 )
   {
      ...
      while ( ringRead != ringWrite )
      {
         ...
         if ( sidDACMode && !( ringBuf[ ringRead ] & ( 1 << 15 ) ) )
         {
            ...
            else if ( sidDACMode == SID_DAC_MONO8 && reg == 0x18 )
            {
      ...
      if ( lastSIDEmulationCycle < curCycleCount )
      {
         ...
         if ( FM_ENABLE )
         {
            emulateCyclesReSIDSingle( cyclesToEmulate );
         }
         else
#endif
         {
...
static inline void handleReadFromSID(int Address)
{
...
static inline void handleWriteToSID( int Address, uint8_t Data )
{
   // Address bit 15 set indecates 2nd SID
...
static inline void handleReadFromConfig(int Address)
{
   gpio_set_dir_masked( 0xff, 0xff );
...
static inline void handleWriteToConfig(int Address, uint8_t Data)
{
   register uint32_t g;
      ...
      */
      ...
      if (SID_ACCESS(g))
      {
         ...
         case 0x20:
            if ( READ_ACCESS( g ) )
            {
               handleReadFromSID(A+34);
               disableDataLines = 1;
            }
            else
            {
               handleWriteToSID( (1 << 15) | A, D );  // 1<<15 = 2nd SID
               shadow_reg[A+32]=D;
            }
            break;
         case 0x40:
         case 0x60:
            /* handle OPL */
            break;
         /* Address $d4c0 - $d4df , 32 raw config-register */
         case 0xc0:
            if ( READ_ACCESS( g ) )
            {
               handleReadFromConfig( A );
            }
            else
            {
               handleWriteToConfig( A, D );
            }
            break;
