...
static float pio_div(float freq, uint16_t* div, uint8_t* frac) {
 ...
 void calc_clocks(const i2s_config* config, pio_i2s_clocks* clocks) {
 ...
 bool validate_sck_bck_sync(pio_i2s_clocks* clocks) {
...
static void dma_double_buffer_output_init(pio_i2s* i2s, void (*dma_handler)(void)) {
    // Set up DMA for PIO I2s - single channel, output only
...
static void i2s_sync_program_init(PIO pio, const i2s_config* config, pio_i2s* i2s) {
    ...
    if (config->din_pin) { /*can't use GIPO 0 as input pin*/
...
void i2s_program_start_output(PIO pio, const i2s_config* config, void (*dma_handler)(void), pio_i2s* i2s) {
    ...
    i2s_sync_program_init(pio, config, i2s);
    dma_double_buffer_output_init(i2s, dma_handler);
    pio_enable_sm_mask_in_sync(i2s->pio, i2s->sm_mask);
}
