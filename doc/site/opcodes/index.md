...
- ExtraCycles: extra cycles taken when crossing a page and/or taking a branch
- MXE: (65816 only) add an extra byte for 16 bit A(1) or X,Y(2)?
  Or is there any other specific behaviour bound to "E"-flag(4)?
- Jump: everything where PC will (or might) not be at PC+Bytes after execution
  (brk, jsr, jmp, rts, branch, etc.)
